<script setup>
	...
	const form=useForm({
        ...
        name_mm : props.type.name_mm
    })

	const updateType = () => {
		form.put(route(`system_dashboard.types.update`, props.type), {
			errorBag: "updateType",
			preserveScroll: true,
		});
	};
...
<template>
	<AppLayout title="Type">
		<template #header>
			<FormHeader title="Edit Type Form" />
		</template>
		{{form}}
		<FormSection @submit.prevent="updateType">
			<template #title>Edit Type</template>
