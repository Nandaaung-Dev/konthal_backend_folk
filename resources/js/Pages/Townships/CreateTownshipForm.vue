<script setup>
	...
	const createTownship = () => {
		form.post(route("system_dashboard.townships.store"), {
			errorBag: "createTownship",
			preserveScroll: true,
		});
	};
</script>
<template>
	<AppLayout title="Township">
		<template #header>
			<FormHeader title="Create Township Form" />
		</template>
		{{form}}
		<FormSection @submit.prevent="createTownship">
			<template #title>Create Township</template>
