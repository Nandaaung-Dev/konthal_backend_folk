<script setup>
	...
	const createShopType = () => {
		form.post(route("system_dashboard.shop_types.store"), {
			errorBag: "createShopType",
			preserveScroll: true,
		});
	};
</script>
<template>
	<AppLayout title="ShopType">
		<template #header>
			<FormHeader title="Create ShopType Form" />
		</template>
		<FormSection @submit.prevent="createShopType">
			<template #title>Create ShopType</template>
