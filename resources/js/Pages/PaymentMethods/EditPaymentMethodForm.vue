<script setup>
	...
	const form = useForm({
		...
		name_mm: props.payment_method.name_mm,
		description: props.payment_method.description,
	});

	const updatePaymentMethod = () => {
		form.put(
			route(`system_dashboard.payment_methods.update`, props.payment_method),
			{
				errorBag: "updatePaymentMethod",
				preserveScroll: true,
			}
		);
	};
...
<template>
	<AppLayout title="PaymentMethod">
		<template #header>
			<FormHeader title="Edit PaymentMethod Form" />
		</template>
		<FormSection @submit.prevent="updatePaymentMethod">
			<template #title>Edit PaymentMethod</template>
