<script setup>
	...
	const form = useForm({
		...
		name_mm: props.city.name_mm,
		region_id: props.city.region_id,
	});

	const updateCity = () => {
		form.put(route(`system_dashboard.cities.update`, props.city), {
			errorBag: "updateCity",
			preserveScroll: true,
		});
	};
...
<template>
	<AppLayout title="City">
		<template #header>
			<FormHeader title="Edit City Form" />
		</template>
		{{form}}
		<FormSection @submit.prevent="updateCity">
