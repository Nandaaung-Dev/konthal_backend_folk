<script setup>
	...
	const form = useForm({
		...
		name_mm: props.branch.name_mm,
		phone_number: props.branch.phone_number,
		address: props.branch.address,
		description: props.branch.description,
		shop_id: props.branch.shop_id,
		shop_type_id: props.branch.shop_type_id,
		city_id: props.branch.city_id,
		township_id: props.branch.township_id,
	});

	const updateBranch = () => {
		form.put(route(`system_dashboard.branches.update`, props.branch), {
...
<template>
	<AppLayout title="Branch">
		<template #header>
			<FormHeader title="Edit Branch Form" />
		</template>
		{{form}}
		<FormSection @submit.prevent="updateBranch">
			<template #title>Edit Branch</template>
			...
			<template #form>
				<div class="col-span-6 sm:col-span-4">
					...
					<InputLabel for="shop_id" value="Shop" />
					<Selector
						id="shop_id"
						type="text"
						class="block w-full mt-1"
						...
						:options="$page.props.shops"
					/>
					<InputError :message="form.errors.shop_id" class="mt-2" />
