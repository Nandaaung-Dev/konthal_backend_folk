<script setup>
	...
	const createProviderBranch = () => {
		form.post(route("system_dashboard.provider_branches.store"), {
			errorBag: "createProviderBranch",
			preserveScroll: true,
		});
	};
</script>
<template>
	<AppLayout title="ProviderBranch">
		<template #header>
			<FormHeader title="Create ProviderBranch Form" />
		</template>
		<FormSection @submit.prevent="createProviderBranch">
			<template #title>Create Provider Branch</template>
