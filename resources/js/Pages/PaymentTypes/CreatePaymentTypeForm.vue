<script setup>
	...
	const createPaymentType = () => {
		form.post(route("system_dashboard.payment_types.store"), {
			errorBag: "createPaymentType",
			preserveScroll: true,
		});
	};
</script>
<template>
	<AppLayout title="PaymentType">
		<template #header>
			<FormHeader title="Create PaymentType Form" />
		</template>
		<FormSection @submit.prevent="createPaymentType">
			<template #title>Create PaymentType</template>
